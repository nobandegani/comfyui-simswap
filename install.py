...
model_url = "https://huggingface.co/netrunner-exe/Insight-Swap-models-onnx/blob/main/simswap_512_beta.onnx"
model_name = os.path.basename(model_url)
models_dir_path = os.path.join(models_dir, "simswap")
...
def download(url, path, name):
    ...
    with tqdm(total=total, desc=f'[SimSwap] Downloading {name} to {path}', unit='B', unit_scale=True, unit_divisor=1024) as progress:
...
with open(req_file) as file:
    try:
        ...
        print(e)
        print(f"Warning: Failed to install {ort}, Simswap will not work.")
        raise e
    strict = True
    for package in file:
        ...
        try:
            ...
            print(e)
            print(f"Warning: Failed to install {package}, Simswap will not work.")
            raise e
print("Ok")
